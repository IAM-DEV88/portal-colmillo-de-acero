...
export const POST: APIRoute = async ({ request, cookies }) => {
    try {
        ...
        const parsedRaidId = (raid_id !== undefined && raid_id !== null && raid_id !== '')
        ...
        if (action === 'create') {
            ...
            if (!baseData.raid_id) {
                return new Response(
                    JSON.stringify({ success: false, error: 'Falta el ID de la raid' }),
                    { status: 400, headers: { 'Content-Type': 'application/json' } }
                );
            }
        ...
        return new Response(
            JSON.stringify({ success: true, message: 'Registro actualizado correctamente', data: result }),
